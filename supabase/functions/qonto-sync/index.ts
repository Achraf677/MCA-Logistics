...
Deno.serve(async (req: Request) => {
  ...
  if (!slug || !secret) {
    return jsonResponse({ ok: false, error: 'missing QONTO_SLUG or QONTO_SECRET_KEY' }, 500);
  }
  ...
  try {
    ...
    for (const account of bankAccounts) {
      ...
      if (transactions.length > 0) {
        ...
        const { error: tErr } = await supabase
          ...
          .upsert(rows, { onConflict: 'qonto_id', ignoreDuplicates: true });
    ...
    return jsonResponse({
      ok: true,
      data: { snapshots, transactions_upserted: transactionsUpserted, balance_cts: balanceCts },
    });
  } catch (err) {
