...
Deno.serve(async (req)=>{
  ...
  const { data: deliveries, error: dErr } = await supabase.from('deliveries').select('id, pennylane_invoice_id').eq('statut', 'facturee').not('pennylane_invoice_id', 'is', null);
  if (dErr) {
    return jsonResponse({
      ok: false,
      error: dErr.message
  ...
  try {
    for (const delivery of deliveries ?? []){
      ...
      const { error: uErr } = await supabase.from('deliveries').update({
        statut: 'payee',
        paid_at: new Date().toISOString()
      }).eq('id', delivery.id).eq('statut', 'facturee');
      if (uErr) {
        return jsonResponse({
          ok: false,
          error: uErr.message
  ...
  return jsonResponse({
    ...
    data: {
      ...
      still_unpaid: checked - markedPayee
    }
  });
});
