        ...
        {tour && tour.status !== 'brouillon' && stops.length > 0 && (
          <Section title="Tournée optimisée">
            ...
            <ol className="flex flex-col">
              {stops.map((s, i) => (
                <li key={s.id} className="flex items-start gap-3 py-2.5 border-b border-[var(--border)] last:border-0">
                  <span className="flex items-center justify-center w-6 h-6 rounded-full shrink-0
                    bg-[var(--brand-soft)] text-[var(--brand)] text-[var(--fs-xs)] font-bold">
                    {s.stop_order ?? i + 1}
                  </span>
                  <div className="flex flex-col min-w-0 flex-1">
                    <span className="text-[var(--fs-sm)] text-[var(--text)] truncate">{s.clients?.name ?? '—'}</span>
                    <span className="text-[var(--fs-xs)] text-[var(--text-muted)] truncate">{s.delivery_address ?? '—'}</span>
                  </div>
                  {s.arrival_time && (
                    <span className="font-mono text-[var(--fs-xs)] text-[var(--text-muted)] shrink-0">
