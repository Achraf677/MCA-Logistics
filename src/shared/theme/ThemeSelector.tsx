    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
      {THEMES.map(t => (
        <button key={t.id} type="button"
          ...
          className={`text-left rounded-[var(--r-lg)] border p-3 press ${active === t.id ? 'border-[var(--brand)] ring-1 ring-[var(--brand)]' : 'border-[var(--border)] hover:border-[var(--border-strong)]'}`}>
